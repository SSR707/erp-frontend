...
import { usePostCartePaymentStudent } from "../service/mutation/usePostCartePaymentStudent";
...
const StudentPaymentCreate = ({ student_id, isOpen, setIsOpen }: IProp) => {
  const [form] = Form.useForm();
  const [api, contextHolder] = notification.useNotification();
  const { mutate: createPayment, isPending } = usePostCartePaymentStudent();
  ...
  const onFinish: FormProps<FieldType>["onFinish"] = (values) => {
    ...
    createPayment(PaymentData, {
      onSuccess: () => {
        ...
        setIsOpen();
        form.resetFields();
        client.invalidateQueries({ queryKey: ["student", student_id] });
      },
      onError: (err: any) => {
        api.error({
...
export default StudentPaymentCreate;
